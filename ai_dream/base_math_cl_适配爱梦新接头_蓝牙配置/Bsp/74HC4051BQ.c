        ...
        case SINGLE_VCC_READ:
            for (count = 0; count < 8; count ++)
            {
                ...
                for (count_1 = 0; count_1 < 8; count_1 ++)
                {
                    ...
                    process_handle->adc_raw_value[count_y][x_array] = GetAdcValue(adc_rank[count_1]) * VOLT_MULT;
                    if (process_handle->adc_raw_value[count_y][x_array] > 254)
                        process_handle->adc_raw_value[count_y][x_array] = 254;
                }
            }
            break;
