...
volatile uint16_t adc_value[16][5] = {0x00};
float adc_v[8] = { 0 };

uint16_t read_adc_value(uint8_t index)
{
    uint32_t sum = 0;

	for(int j = 0; j < 8; ++j)
    {
        sum += adc_value[j][index];
	}
    return sum / 16;
}
